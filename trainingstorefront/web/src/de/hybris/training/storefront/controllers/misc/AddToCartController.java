...
import de.hybris.training.core.services.impl.DefaultTrainingCartService;
...
public class AddToCartController extends AbstractController {
	...
	private static final Logger LOG = Logger.getLogger(String.valueOf(AddToCartController.class));

	@Resource
	private ModelService modelService;

	@Resource(name = "defaultTrainingCartService")
	private DefaultTrainingCartService cartService;
	...
	@RequestMapping("cart/entry/duplicate/{entryNumber}")
	public String duplicateCartEntry(@PathVariable int entryNumber, RedirectAttributes model) {
		try {
			//set new entry to cart
			cartService.addEntryToCart(entryNumber, model);
			GlobalMessages.addFlashMessage(model, "accErrorMsgs", "basket.page.message.duplicate.success");
		}catch(UnknownIdentifierException e){
			GlobalMessages.addFlashMessage(model, "accErrorMsgs", "basket.page.message.duplicate.failed");
		}
